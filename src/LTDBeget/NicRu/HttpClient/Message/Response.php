...
namespace LTDBeget\NicRu\HttpClient\Message;
...
class Response extends BaseResponse
{
	...
	public function getArrayContent()
	{
		...
		if ($this->getHeader("Content-Type") === "text/plain") {
			...
			if (count($content) == 0) {
				return ['state' => '404', 'statemsg' => 'Error', 'request-id' => '', 'type' => 'error', 'body' => '',];
			}
			...
			if (count($content) <= 1) {
				return ['state' => $state, 'statemsg' => $statemsg, 'request-id' => '', 'type' => 'error', 'body' => '',];
			}
			$requestId = trim($content[1]);
			$requestId = substr($requestId, 12);
			if (count($content) <= 4) {
				return ['state' => $state, 'statemsg' => $statemsg, 'request-id' => $requestId, 'type' => 'error', 'body' => '',];
			}
			$type = trim($content[4]);
			$type = substr($type, 1);
			$type = substr($type, 0, -1);
			if (count($content) <= 5) {
				return ['state' => $state, 'statemsg' => $statemsg, 'request-id' => $requestId, 'type' => $type, 'body' => '',];
			}
			$body = [];
